        ...
        while not (quit_keypress() or ctrl_c):
            if observer.rgb_image is not None:
                ...
                if observer.eye_image is not None:
                    ...
                    row, col = (int(gaze_projection[0]), 1408 - 1 - int(gaze_projection[1]))
                    rgb_image = np.rot90(rgb_image, -1).copy()
                    cv2.rectangle(rgb_image, (col-w//2, row-w//2), (col+w//2, row+w//2), color=(0,255,0), thickness=2)

                    try:
                        results = model(rgb_image[row-w//2:row+w//2+1, col-w//2:col+w//2+1], verbose=False)
                        for result in results:
                            ...
                            for box in boxes:
                                if box.conf[0].item() > 0.7:
                                    ...
                                    cv2.rectangle(rgb_image, (col-w//2+int(x1), row-h//2+int(y1)), (col-w//2+int(x2), row-h//2+int(y2)), color=(0, 0, 255), thickness=2)
                                    cv2.putText(rgb_image, label, (col-w//2+int(x1), row-h//2+int(y1)-30), cv2.FONT_HERSHEY_DUPLEX, 2.5, (255, 255, 255), 2, cv2.LINE_AA)
                    except:
                        pass
